   ...
   if (self) {
      ...
      if (event.hasRecurrenceRules) {
         ...
         for (EKRecurrenceRule *rule in event.recurrenceRules) {
            if (rule.frequency == EKRecurrenceFrequencyDaily) self.repeatString = @"Every day";
            else if (rule.frequency == EKRecurrenceFrequencyWeekly) self.repeatString = @"Every week";
            else if (rule.frequency == EKRecurrenceFrequencyMonthly) self.repeatString = @"Every month";
            else if (rule.frequency == EKRecurrenceFrequencyYearly) self.repeatString = @"Every year";
         }
      }
      ...
      if (event.hasAlarms) {
         ...
         for (EKAlarm *alarm in event.alarms) {
            if (alarm.relativeOffset == 0.0) self.alarmString = @"At time of event";
            else if (alarm.relativeOffset == -300.0) self.alarmString = @"5 minutes before";
            else if (alarm.relativeOffset == -600.0) self.alarmString = @"10 minutes before";
            else if (alarm.relativeOffset == -1800.0) self.alarmString = @"30 minutes before";
            else if (alarm.relativeOffset == -3600.0) self.alarmString = @"1 hour before";
         }
      }
   ...
   if (indexPath.section == 0) {
      ...
      if (indexPath.row == 0) {
         ...
         if (self.didPushRepeatAlertVC && self.eventTitle) {
            ...
            eventTextFieldCell.titleTextField.text = @"New Title";
